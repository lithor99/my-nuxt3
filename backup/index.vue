        ...
        <div class="max-w-6xl mx-auto bg-white shadow-md rounded-lg p-6">
            <div class="flex justify-between items-center mb-4">
                <div class="relative flex justify-start items-center">
                    ...
                    <div class="relative w-60 mx-auto ml-2">
                    ...
                    <!-- <div class="relative w-40 ml-2">
                        <input
                            v-model="selectedDate"
                            type="date"
                            class="w-full border border-gray-400 rounded-lg shadow-sm focus:border-blue-500 px-4 py-2"
                        />
                    </div>
                    <div class="relative w-40 ml-2">
                        <input
            ...
            <div class="flex items-end justify-between">
                <div class="text-gray-500 mt-0 flex items-center">
                    ...
                    <span class="text-gray-800 ml-2">
                        {{ printQrStore.getPagination?.count ?? 0 }}
                    </span>
                </div>
            </div>
            ...
            <div v-else class="overflow-x-auto">
                <table class="w-full border-collapse border rounded-md border-gray-200">
                    ...
                    <tbody>
                        <tr v-for="(item, index) in printQrStore.getPagination
                        ...
                        <tr v-if="printQrStore.getPagination.count === 0">
            ...
            <div class="flex items-center justify-between mt-4 space-x-2">
                ...
                <div class="flex items-center">
                    <span v-if="printQrStore.getPagination?.count > 0" class="text-gray-700 mr-4">Page {{ currentPage }}
                        of
                        {{ printQrStore.getPagination?.totalPage ?? 0 }}</span>
                    ...
                    <button :disabled="currentPage ===
                        printQrStore.getPagination?.totalPage ||
                        printQrStore.getPagination?.count === 0
                        "
            ...
            <div class="mt-4 flex justify-end">
                <button
                    ...
                    @click="
                        onPrintMultiQR(printQrStore.getPagination.rows ?? [])
                        ">
...
const headers = ref([
    ...
    { title: "ພີມໂດຍຜູ້ໃຊ້", key: "printBy", align: "start" },
    { title: "ວັນທີ່ພີມ", key: "createdAt", align: "start" },
]);
...
onMounted(() => {
    ...
    if (!isQueryServerSide) {
        loading.value = true;
        isFirst.value = true;
        printQrStore.setPagination(filters.value);
        loading.value = false;
        isFirst.value = false;
    }
...
const onSearch = async () => {
    ...
    await printQrStore.setPagination(filters.value);
    setQueryServerSide(false);
};
...
const onNextPage = async () => {
    if (currentPage.value < printQrStore.getPagination?.totalPage) {
        currentPage.value++;
        currentPageCookie.value = currentPage.value;
        await updatePagination();
    }
...
const onLastPage = async () => {
    if (currentPage.value < printQrStore.getPagination?.totalPage) {
        currentPage.value = printQrStore.getPagination?.totalPage;
        currentPageCookie.value = currentPage.value;
        await updatePagination();
    }
};
...
const generatePageNumbers = () => {
    const totalPage = printQrStore.getPagination?.totalPage ?? 0;
    const pages = [];
